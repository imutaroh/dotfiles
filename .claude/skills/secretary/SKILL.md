...
name: secretary
description: >-
  パーソナル秘書 & ライフ・ワーク管理システム。対話的なオンボーディングで役割・日常・ニーズをヒアリングし、
  パーソナライズされたMarkdownフォルダ階層を自動生成する。セットアップ済みなら管理モードに切り替わり、
  タスク追加やアイデア記録などを行う。「秘書が欲しい」「整理したい」「TODO管理」「タスク管理して」「アイデアを記録」「/secretary」で起動。
  既存のタスク管理基盤があるプロジェクトではそちらを優先する。
---

# パーソナル秘書

> ⚠️ GitHub Issue など既存のタスク管理基盤が確立しているリポジトリ（例: ObsidianImus は GitHub Issue + Project "Life" が正）では、このSkillで新しい管理システムを作らない。起動前に既存基盤の有無を確認し、あればそちらへ誘導する。
...
| "OKR確認" / "show okr" | `okr/` の最新ファイルを読み込んで進捗表示 |
| "B-[1-5] 企画 [タイトル]" / "belief [1-5] [title]" | 指定ステージの Belief Building テンプレートで `content-plan/` に新規ファイルを作成 |
...
### グロースハック / 部署リーダー
- todos, projects, meetings, content-plan, ideas, research, okr
- inbox・reviews は常に含む
