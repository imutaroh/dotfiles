...
1. 対象日の各セッションを scratchpad に書き出す:
   ...
   ```
2. ファイルサイズを見て 3〜6 体の Explore サブエージェントに分担（1体あたり合計 100〜150KB 目安。巨大ファイルは単独割当）。**必ず `model: "sonnet"` を指定する**（transcript 要約は単純作業。Fable/Opus を使うと枠を無駄に食う。2026-07-24 にモデル未指定＝Fable で実行してしまった反省から固定）
