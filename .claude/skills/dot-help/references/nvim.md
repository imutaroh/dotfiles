# Neovim（AstroNvim）の調べ方

- 構成: **AstroNvim v5** ベース。**Leader = Space**
- 設定の場所: `~/dotfiles/.config/nvim`（必ず絶対パスで参照。カレントディレクトリに依存しない）
...
| `plugins/*.lua` の `keys = {}` | プラグイン固有キー（下のトピック別ルーティング参照） |
...
| lazygit | `lazygit.lua` |
| hunk 単位の git 操作・インライン diff | `gitsigns.lua` |
...
| フォーマッタ・リンタ | `conform.lua`, `mason.lua` |
