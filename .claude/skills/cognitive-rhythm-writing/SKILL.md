...
- 冒頭の仕事は、最初の数文で未回収の緊張を一つ作ることである。**型は問わない**。使える型の例：
  ...
  - 普遍的な場面のセリフ一行（「今期、あなたの成果は何でしたか？」）で開き、直後に読者の記憶にある失敗の追体験（「〜と答えた瞬間の、あの微妙な空気」）を置く。緊張は場面が作るので、問題提起の文を足さない
...
- 見出しも本文と同じく緊張の装置である。議題表型（「生産性測定について」「DORAメトリクスの紹介」）にせず、断定形（「測った瞬間に指標は壊れる」）・逡巡形（「『たくさん書いた』は成果なのか」）・セリフ形（「今期、あなたの成果は何でしたか？」）で、読む前に反問を起こさせる。
- ただし断定形見出しは、本文がその断定を実際に試すことが条件。本文が試さない断定見出しは、釣りであって緊張ではない。
- リファレンス用途の文書（読者が目次から目的の節へ跳ぶもの）では議題表型が正しい。この設計は通読させたい読み物にだけ適用する。
...
- 着地の最も強い型は円環である。冒頭に置いた失敗例を、本文の主張を適用したあとの姿に**書き換えて再提示**する（「PRを120個マージしました」→「決済のリトライ処理を改善して、月次の問い合わせが半減しました」）。読者は抽象を覚えて帰るのではなく、書き換えの前後差分を覚えて帰る。冒頭を場面で開いた文章では、この型を第一候補にする。
...
この規範の装置が揃って機能している公開記事。診断や生成で迷ったら読み直す。

- [エンジニアの成果はなぜ測れないのか（zenn.dev/awesome_kou）](https://zenn.dev/awesome_kou/articles/engineer-performance-metrics) — セリフ一行＋追体験の冒頭、断定形・逡巡形の見出し、節間を反問だけで繋ぐ運び、冒頭の失敗例を修正版に書き換える円環の結び、がすべて入っている。論争のある大きめのテーマだから機能している点にも注意（手順解説の短い記事で反問を連発すると回りくどくなる）。
